...
SELECT TOP 1 billing_city, SUM(CAST(total AS DECIMAL(10, 2))) AS InvoiceTotal
FROM invoice
...
SELECT top 1 customer.customer_id, customer.first_name, customer.last_name, SUM(CAST(total AS DECIMAL(10, 2))) AS total_spending
FROM customer 
...
WITH Customter_with_country AS (
		SELECT customer.customer_id,customer.first_name,customer.last_name,invoice.billing_country, SUM(CAST(total AS DECIMAL(10, 2))) AS total_spending,
	    ROW_NUMBER() OVER(PARTITION BY billing_country ORDER BY SUM(CAST(total AS DECIMAL(10, 2))) DESC) AS RowNo 
		FROM invoice
		JOIN customer ON customer.customer_id = invoice.customer_id
		WHERE TRY_CAST(total AS DECIMAL(10, 2)) IS NOT NULL
		GROUP BY 1,2,3,4
